...
export class TagBuilderWriter {
    codeBlockWriter: CodeBlockWriterLike

    constructor(codeBlockWriter: CodeBlockWriterLike) {
        this.codeBlockWriter = codeBlockWriter
    }

    attribute(name: string, value: string) {
        this.codeBlockWriter.write(' ')
        this.codeBlockWriter.write(name)
        this.codeBlockWriter.write('="')
        this.codeBlockWriter.write(value)
        this.codeBlockWriter.write('"')
    }
}

export type Callback<T extends TagBuilder> = (tag: T) => void

export class TsxWriter {
    ...
    constructor(codeBlockWriter: CodeBlockWriterLike) {
        this.codeBlockWriter = codeBlockWriter
        this.tagBuilder = new TagBuilderWriter(this.codeBlockWriter)
    }

    tag(name: string, build: Callback<TagBuilder>) {
        this.codeBlockWriter.write(`<${name}`)
        build(this.tagBuilder)
        this.codeBlockWriter.write(`>`)
        this.codeBlockWriter.write(`</${name}>`)
    }
}
...
export interface CodeBlockWriterLike {
    /**
