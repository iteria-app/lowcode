...
function updateReducer(reducer, initialArg, init) {
  ...
  if (first !== null) {
    ...
    // Mark that the fiber performed work, but only if the new state is
    // different from the current state.
    if (_newState !== hook.memoizedState) {
      markWorkInProgressReceivedUpdate();
    }
...

{
  ...
  var warnInvalidHookAccess = function () {
    warning$1(false, 'Hooks can only be called inside the body of a function component. ' + 'Do not call Hooks inside other Hooks. For more information, see ' + 'https://fb.me/rules-of-hooks');
  };
