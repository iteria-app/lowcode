...
self.addEventListener('fetch', function (event) {
  ...
  if (requestURL.pathname.startsWith(CONTROLLED + 'loading.html')) {
    event.respondWith(fetch('/loading.html'))
  } else if (requestURL.pathname.startsWith(CONTROLLED)) {
    event.respondWith(caches.open('playground').then(async (cache) => {

    if (requestURL.pathname.endsWith('.svelte') || requestURL.pathname.endsWith('.js') || requestURL.pathname.endsWith('.ts') || requestURL.pathname.endsWith('.jsx') || requestURL.pathname.endsWith('.tsx')) {
    ...
    async function fixJsResponse(response) {
      ...
      return newJavaScriptResponse(text
        ...
        .replace('https://unpkg.com/intl-messageformat@^9.3.15?module', 'https://unpkg.com/intl-messageformat@9.3.15/lib/index.js?module')
        .replace('https://unpkg.com/intl-messageformat-parser@6.0.13?module', 'https://unpkg.com/intl-messageformat-parser@6.0.13/lib/index.js?module')
        )
    }
    event.respondWith(fetch(event.request.url, {redirect: 'follow'}).then(fixJsResponse).catch(err => {
      //fallback
      const newUrl2 = "https://cdn.skypack.dev" + requestURL.pathname
      return fetch(newUrl2, {redirect: 'follow'}).then(fixJsResponse).catch(err2 => {
