...
importScripts(
  'https://unpkg.com/svelte@3.29.4/compiler.js',
...
self.addEventListener('fetch', function (event) {
  ...
  if (c) {
    ...
    /**/ if (requestURL.pathname.startsWith(CONTROLLED)) {
      ...
      const response = fetchFile(gitlabUrl, '4rT2D1sVBmc-iE2L3AEu').then(
        (svelteSource) => {
          console.log('CO JE TENTO OBJEKT', svelteSource);
          //TODO if *.ts esbuildPromise.then(transpiler.transform('enum Foo { A, B }', { loader: 'ts' })

          const aliases = {
            '@urql/svelte':
              'https://unpkg.com/@urql/svelte@1.1.2/dist/urql-svelte.js',
            '@material/mwc-': 'https://unpkg.com/@material/mwc-',
            'svelte-i18n':
              'https://unpkg.com/browse/svelte-i18n@3.1.0/dist/runtime.esm.js',
            'graphql-request':
              'https://unpkg.com/graphql-request@3.3.0/dist/index.js',
          };
