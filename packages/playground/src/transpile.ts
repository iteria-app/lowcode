...
export async function transpileSvelte(source: string, filename: string): Promise<Transpiled> {
  const matches = scriptTs.exec(source)
  if (matches?.length) {
    const endMatches = scriptEnd.exec(source)
    const ts = source.substring(matches.index + matches[1].length, endMatches?.index)

    const transpiler = await esbuildPromise
    const jsSource = await transpiler.transform(ts, { loader: 'ts' })
    if (jsSource.code.length < ts.length) {//TODO consider source map
  ...
  const path = filename.endsWith('.svelte') ? filename.substring(0, filename.length - '.svelte'.length) : filename
  return { code: compiledCode, path, warnings: [/* TODO */] }
}

export function transpileEsbuild(source: string, filename: string): Promise<Transpiled> {
  const dot = filename.lastIndexOf('.')
  const hasExtension = dot > 0 && dot < filename.length
  const extenstion = hasExtension ? filename.substring(dot + 1) : 'ts'
  const path = hasExtension ? filename.substring(0, filename.length - extenstion.length - 1) : filename
  return esbuildPromise.then((transpiler: any) => {
    return transpiler.transform(source, { loader: extenstion }).then((transpiled: any) => {
