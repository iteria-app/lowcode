...
browser.runtime.onMessage.addListener((msg, sender) => {
  ...
  if (msg?.payload?.value?.source?.fileName) {
    const fileUrl = 'file:///' + msg?.payload?.value?.source?.fileName.replaceAll('\\', '/')
    
    fetchLocalCode(fileUrl).then((resp)=>{
      console.log('background.js fetch file reponse', resp, msg?.payload?.value?.source?.fileName)
      notifyDevtools({event: 'inspectedElementSource', status: 'ok', fileUrl, body: resp?.body});
    }).catch(err => {
      console.error('background.js failed to fetch file', err, msg?.payload?.value?.source?.fileName)
      notifyDevtools({event: 'inspectedElementSource', status: 'error', error: err, errorMessage: err + '', fileUrl});
    })
  }
