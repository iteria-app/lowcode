...
export async function tsClone(code: string, source: SourceLineCol, filePath: string) {
  const found = await astFindSource(code, source);
  if (found) {
    if (found) {
      console.log('source found', found, code.substring(found.pos, found.end), filePath);
    }
